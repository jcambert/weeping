...
module.exports = {
  ...
  registerDatastore: function (datastoreConfig, physicalModelsReport, done) {
    ...
    setTimeout(() => {
     ...
     // console.log(datastoreConfig);
      ...
      //
      //sails.log.debug(datastoreName);
      //sails.log.debug(sails.datastores[datastoreName].manager);
  ...
  create: function (datastoreName, query, done) {
    ...
    var rootRef=db.ref(datastoreName);
    var newPostRef = rootRef.child(query.using).push();
    var postId = newPostRef.key;
    query.newRecord.id=postId;
    newPostRef.set(query.newRecord,error=>{
      if(error)
        {return done(new Error(error));}
      else {
        //console.log(query.newRecord);
        return done(null,query.meta.fetch?query.newRecord:undefined);
      }
    });
  ...
  destroy: function (datastoreName, query, done) {
    ...
    var ref = db.ref(datastoreName);
    ref.child(query.using).remove();
    //console.log(query);
    return done();
    // Perform the query (and if relevant, send back a result.)
    //
  ...
  find: function (datastoreName, query, done) {
    ...
    if(query.criteria.where.and || query.criteria.where.or)
      {return done(new Error('In firebase, only one criteria in where clause are authorized'));}

    var key;
    var keys=_.keys( sails.models[query.using].attributes);
    //sails.log(keys);
    //sails.log(query.criteria.select);
    //return done(null,[]);
    var rootRef=db.ref(datastoreName);
    var req=rootRef.child(query.using);
    //sails.log('Criteria where length:'+_.size(query.criteria.where));

    if(_.size(query.criteria.where)==1){
      key=_.keys(query.criteria.where)[0];
      req=req.orderByChild(key).equalTo(query.criteria.where[key]);
    }

    if(query.criteria.skip && query.criteria.skip>0)
      {req=req.startAt(query.criteria.skip);}

    if(query.criteria.limit)
      {req=req.limitToFirst(query.criteria.limit-1);}

    req.once('value',snapshot=>{
      var result=snapshot.val();
      if(keys.length!=query.criteria.select.length)
        {_.forEach(result,(v,k)=>{
          result[k]=_.pick(v,query.criteria.select);
        })};
  ...
  count: function (datastoreName, query, done) {
    ...
    //sails.log('datastoreName :'+datastoreName);
    //sails.log(query);
    // Look up the datastore entry (manager/driver/config).
    var dsEntry = registeredDatastores[datastoreName];
    ...
    req.once('value',snapshot=>{
      ...
      return done(null,_.keys(result).length);
    },error=>{
      return done(new Error(error));
    });
  ...
  drop: function (datastoreName, tableName, unused, done) {
    //sails.log.debug('Attemting to drop '+datastoreName+':'+tableName);

    //sails.log.debug(sails.models[tableName]);
    // Look up the datastore entry (manager/driver/config).
    var dsEntry = registeredDatastores[datastoreName];
    //console.log(dsEntry);
    ...
    var ref = db.ref(datastoreName);
    ref.child(tableName).remove();
    //sails.log(tableName+" was removed");
    return done();
  ...
  test:function(datastoreName,done){
    return done();
  }
};
